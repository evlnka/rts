 ...
 * 
 * condvar vs poll/ppoll:
 * - condvar: для синхронизации потоков (разделяемая память)
 * - poll/ppoll: для файловых дескрипторов (I/O операции)
 * 
 * condvar vs mq_timedreceive:
 * - condvar: внутрипроцессная коммуникация (потоки)
 * - mq_timedreceive: межпроцессная коммуникация
 * 
 * Особенности condvar:
 * - Требует мьютекса для защиты разделяемого состояния
 * - Подвержена ложным пробуждениям (spurious wakeups)
 * - Использует абсолютное время для таймаутов
 * 
 * Сценарии применения condvar:
 * - Производитель-потребитель между потоками
 * - Ожидание изменения разделяемого состояния
 * - Синхронизация работы пулов потоков
 * - Когда данные уже в памяти, а не в I/O
 */
