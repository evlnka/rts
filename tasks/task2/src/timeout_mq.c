 ...
 * 
 * mq_timedreceive vs poll/ppoll:
 * - mq_timedreceive: специализирован для очередей сообщений
 * - poll/ppoll: универсальные для любых файловых дескрипторов
 * 
 * mq_timedreceive vs condvar:
 * - mq_timedreceive: межпроцессное взаимодействие
 * - condvar: только внутри одного процесса
 * 
 * Особенности mq_timedreceive:
 * - Гарантированная доставка сообщений
 * - Поддержка приоритетов сообщений
 * - Сообщения сохраняются при перезапуске процессов
 * - Требует управления системными ресурсами (удаление очередей)
 * 
 * Сценарии применения mq_timedreceive:
 * - Микросервисная архитектура
 * - Межпроцессное взаимодействие с гарантированной доставкой
 * - Системы с разделением на независимые процессы
 * - Когда нужна буферизация сообщений
 */
