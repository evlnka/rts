 ...
 * 
 * poll vs ppoll:
 * - poll: простая блокировка с таймаутом, уязвима к race condition с сигналами
 * - ppoll: атомарная разблокировка сигналов, безопасна для обработки сигналов
 * 
 * poll vs condvar:
 * - poll: для файловых дескрипторов (сокеты, pipe, файлы)
 * - condvar: для межпоточной синхронизации (разделяемая память)
 * 
 * poll vs mq_timedreceive:
 * - poll: мультиплексирование множества дескрипторов
 * - mq_timedreceive: специализирован для POSIX очередей сообщений
 * 
 * Сценарии применения poll:
 * - Сетевые серверы (множественные соединения)
 * - Ожидание данных от множества источников
 * - Простые случаи без обработки сигналов
 * - Когда нужна кроссплатформенная совместимость
 */
