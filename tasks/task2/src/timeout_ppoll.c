 ...
 * 
 * ppoll vs poll:
 * - ppoll: атомарная работа с сигналами, нет race condition
 * - poll: возможна потеря сигналов между проверкой и блокировкой
 * 
 * ppoll vs condvar:
 * - ppoll: для файловых дескрипторов + безопасные сигналы
 * - condvar: только для потоков, не для межпроцессного взаимодействия
 * 
 * ppoll vs mq_timedreceive:
 * - ppoll: универсальный для любых файловых дескрипторов
 * - mq_timedreceive: только для POSIX очередей сообщений
 * 
 * Сценарии применения ppoll:
 * - Надежные сетевые серверы с обработкой сигналов
 * - Приложения, использующие SIGTERM/SIGINT для graceful shutdown
 * - Real-time системы с обработкой прерываний
 * - Когда критически важна надежность обработки сигналов
 */
...
static void *thread_sender(void *arg) {
    ...
    // Используем getpid() вместо pthread_main_np() для кроссплатформенности
    kill(getpid(), SIGUSR1);
    return NULL;
}
