...
state = 'N';  // Ставим начальное состояние - не готово
...
// Ожидание CTRL + C (остальные потоки будут работать до завершения или до получения сигнала)

pause(); // приостанавливаем выполнения главногот потока до получения сигнала
 
// Сюда поток не дойдёт из-за бесконечных циклов. Для корректного
// завершения используйте булевы флаги вместо констант TRUE.
 
printf("Успешный выход из системы!\n"); 
...
printf("Состояние изменилось. Теперь оно: "); 
